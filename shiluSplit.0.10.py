	...
	if os.path.isfile(os.path.join(slpath, file)) and file != ".DS_Store":
		...
		for line in filein: 
			...
			if t != None:
				...
				entry = entry.strip("○△")
				fileout.write(titl + "," + str(vol) + "," + str(vln) + "," + str(ln) + "," + str(rdate) + ","  + ganzhi + "," + str(date) + "," + file + "," + term + ",")
				entrySplit = entry.split(term)
				entrySplit.reverse()
				while len(entrySplit) > 0:
					fileout.write(entrySplit.pop() + ",")
				fileout.write("\r")
