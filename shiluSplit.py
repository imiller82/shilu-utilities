...
for file in files:
	if os.path.isfile(os.path.join(slpath, file)) and file != ".DS_Store":
		...
		for line in filein: 
			...
			#If the line contains the search term, split and print it with the other info
			...
			if t != None:
				...
				fileout.write(titl + "," + str(vol) + "," + str(vln) + "," + str(ln) + "," + str(rdate) + ","  + ganzhi + "," + str(date) + "," + file + ",")
				fileout.write(entrySplit(entry, term))
				fileout.write("\r")
