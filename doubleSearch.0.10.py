...
if len(sys.argv) == 1:
	...
	searchTerm2 = input("Enter a second search term: ")
elif len(sys.argv) == 2:
	...
	searchTerm = sys.argv[1]
	searchTerm2 = sys.argv[2]
	if ".csv" in sys.argv[3] or ".txt" in sys.argv[3]:
	...
	if os.path.isfile(os.path.join(slpath, file)) and file != ".DS_Store":
		...
		for line in filein: 
			...
			if t != None:
				...
				entry = entry.strip("○△")
				entry = entry.strip(gzRE)
				fileout.write(titl + "," + str(vol) + "," + str(vln) + "," + str(ln) + "," + str(rdate) + ","  + ganzhi + "," + str(date) + "," + file + "," + term + "," + term2 + "," + entry + "\r")
